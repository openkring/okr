...
// reports). `planSelection` and the verbs never call a block’s `.routes()`, so the
...
export {
  createApplyFeatureSelection, planAddMenuRows, planEnableBlock, planSelection,
} from './apply-feature-selection';
export type { SelectionPlan, VerbResult } from './apply-feature-selection';
