...
describe('assertTargetAcceptable', () => {
  ...
  it('still refuses an unroutable model target in a redirect space', () => {
    ...
    expect(() => assertTargetAcceptable(spaceQr({ kind: 'redirect' }), 'model', '', 'location.abc')).toThrow(/detail route/i);
    expect(() => assertTargetAcceptable(spaceQr({ kind: 'redirect' }), 'model', '', 'calevent.abc')).toThrow(/detail route/i);
  });
