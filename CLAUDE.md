...
Environment files (`environment.ts`, `environment.prod.ts`) are generated by `set-env.js` from environment variables or GCP Secret Manager — never commit them. In dev, load variables with `source ./apps/<app>/.env`. The `ENV` injection token provides `BkEnvironment` across the app.

### Cloud Functions

Located in `apps/functions/src/`. Organized into sub-modules: `auth`, `matrix`, `matrix-simple`, `oidc-bridge`, `replication`. Built with esbuild via `pnpm run build:functions`.
