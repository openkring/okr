...
| `logo`                   | working with a tenant logo, app icon, favicon, PWA manifest icons or `apple-touch-icon` — adding/replacing a tenant logo, an app icon that shows a letter monogram, a logo clipped inside a circle, or a proposal to hand-place another `*-icon.png`. Covers the one-master rule (`logo.svg`), the SVG→PNG→imgix chain, why imgix cannot read SVG, the full-bleed requirement, and generated vs. hand-authored assets. |
...
| `authentication`         | a user cannot log in, a password reset does not arrive or does not work, an account is locked out, or wiring login/reset for a new tenant — the two-document rule (`users/{uid}` doc id = Auth uid), the four causes behind one login error message, the five-link reset chain, the project-wide seeclub.org action URL, and handing a new admin a first password.                                              |
...
| `styling`                | a tenant's colors/fonts/visual identity — deriving an app's Ionic palette in `theme/variables.scss` from the website tokens, an app that still looks like Seeclub (green/blue) after provisioning, shade/tint/contrast choices, or linking a brand-styleguide into the app menu. Covers the three styling layers, the scs-palette inheritance trap in the app generator, and the `palette.mjs` tool. |
...
| `authoring-docs`         | creating/saving a spec, design, or implementation plan doc — **and before writing ANY markdown in this repo** (task reports under `.superpowers/sdd/`, READMEs, reference docs): it carries the markdownlint rules these files are linted against (blank line around every heading/list/fence, `\|` inside table cells, fence languages).                                                                                                                                                                                                                                                          |
...
- after editing a `*.validations.ts` or a `*.form.ts`, run `pnpm check-forms`. It guards the rules in the **`building-forms`** skill that decide whether a modal can be saved at all: no length cap on a selector / generated / foreign-key field, a template `[maxLength]` bound to the very constant the Vest suite enforces, and an `okr-error-note` under every validated field. A broken one of these compiles, lints and tests green while the change-confirmation banner silently never appears.
