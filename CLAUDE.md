...
Environment file (`environment.ts`) is generated by `set-env.js` from environment variables or GCP Secret Manager — never commit them. In dev, load variables with `source ./apps/<app>/.env`. The `ENV` injection token provides `OkrEnvironment` across the app.
