...
/**
 * No-op mask for entering a NEW password. PasswordMask silently swallows every character
 * outside its allow-list (# / ~ [ ] { } < > ' " \\ | ^ …) while the user types, so a password
 * generated by iOS or a password manager arrives mangled with no message at all. A password
 * is never parsed or reformatted, so there is nothing for a mask to enforce here — length is
 * checked by the Vest suite instead.
 */
export const AnyCharMask: MaskitoOptions = {
  mask: /^.*$/,
};
