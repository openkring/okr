...
/**
 * Floor for a password being TYPED AT LOGIN. Stays at 6 deliberately: accounts created before
 * the set-password floor was raised still have 6-character passwords, and a stricter login
 * validator would lock those members out of their own account with a form error.
 */
export const PASSWORD_MIN_LENGTH = 6;
/**
 * Floor for a password being SET. Firebase Auth itself rejects anything under 6
 * (auth/weak-password); 8 is what we ask for, checked in the form so the user learns it while
 * typing rather than after a round trip.
 */
export const PASSWORD_SET_MIN_LENGTH = 8;
/**
 * Was 24, which silently truncated passwords generated by iOS or a password manager. Long
 * enough now that no generated password is cut off; nothing parses or stores this field by
 * width, so the cap exists only to bound the input.
 */
