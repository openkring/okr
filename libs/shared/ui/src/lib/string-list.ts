...
export class StringList {
  ...
  protected readonly i18n = this.i18nService.translateAll({
    string_title: PFX + 'string.title',
    string_edit: PFX + 'string.edit',
    string_add: PFX + 'string.add',
    ok: '@ok',
    cancel: '@cancel'
  });

  // computed
  protected cardTitle = computed(() => this.title() || this.i18n.string_title());
  ...
  public async edit(text: string, index: number): Promise<void> {
    const changedText = await okrPrompt(this.alertController, this.i18n.string_edit(), '', this.i18n.ok(), this.i18n.cancel(), text);
    if (changedText) {
      this.strings.update(arr => arr.map((t, i) => i === index ? changedText : t));
    }
  }
