            ...
            @for(inv of invitations(); track inv.okey) {
              <ion-row (click)="showActions(inv)">
                ...
                <ion-col>
                  ...
                  @if(inv.notes) {
                    <ion-label class="invitation-text">{{ inv.notes }}</ion-label>
                  }
                </ion-col>
              </ion-row>
            }
...
export class InvitationsSectionComponent implements OnInit {
  ...
  protected readonly invitations = computed(() => {
    switch (this.scope()) {
        ...
        case 'open':
            return this.store.openInvitations();
        case 'all':
    ...
    if (actionSheetOptions.buttons.length > 0) {
      ...
      switch (data.action) {
        ...
        case 'invitation.unsubscribe':
          ...
          break;
        case 'invitation.openCalevent':
          await this.store.openCalevent(invitation);
          break;
      }
    }
  }
