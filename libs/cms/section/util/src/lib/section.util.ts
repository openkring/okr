...
export function createSection(type: SectionType, tenantId: string): SectionModel {
  ...
  switch (type) {
    ...
    case 'sankey': section = { ...SANKEY_SECTION_SHAPE } as SankeySection; break;
    ...
    case 'weather':
      // `properties` is cloned for the same reason as 'article' above, and one reason more:
      // WeatherConfig carries `locationKeys`, an ARRAY. A bare spread would hand every new
      // weather section the SAME array instance, so picking locations for one map section
      // would silently add them to every other one created in the session.
      section = {
        ...WEATHER_SECTION_SHAPE,
        properties: { ...WEATHER_CONFIG_SHAPE, locationKeys: [] },
      } as WeatherSection;
      break;
    default:
...
export function narrowSection(section: any): SectionModel | undefined {
  switch (section.type) {
    ...
    case 'member-age': return section as MemberAgeSection;
    case 'member-cat': return section as MemberCatSection;
    case 'sankey': return section as SankeySection;
    ...
    case 'weather': return section as WeatherSection;
...
export function getSectionIndex(section: SectionModel): string {
  return 'n:' + section.name + ' t:' + section.type +
    ' tt:' + buildSearchTokens(section.title) +
    ' st:' + buildSearchTokens(section.subTitle);
}

export function getSectionIndexInfo(): string {
  return 'n:name t:type tt:titleTokens st:subTitleTokens';
}
