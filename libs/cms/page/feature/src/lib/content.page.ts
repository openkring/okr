...
export class ContentPage {
  ...
  protected hiddenMenuItems = computed(() => [
    ...
    // Uploading needs an album to upload into — AND a folder the user may write to. The
    // second half mirrors the `docs` create rule (see canUploadIntoFolder): without it a plain
    // member was offered the upload on any album and got "Missing or insufficient permissions"
    // once the files were already picked.
    ...(this.albumSection()?.canUpload() ? [] : ['files-add']),
   ...
   * page that actually has an album: the menu then offers them print, the raw export and — on a
   * folder that permits it (`canUpload`) — the file upload, while every structural item — the edit-mode toggle itself,
