 ...
 * One warning per mismatching pair, naming the newer line; never blocks anything. A pair in two
 * currencies (the section boundary of a multi-currency statement, Swissquote) is not compared.
 */
export function checkSaldo(rows: ParsedRow[], newestFirst = true): ParsedWarning[] {
  const out: ParsedWarning[] = [];
  for (let i = 0; i + 1 < rows.length; i++) {
    const a = rows[i], b = rows[i + 1];
    if (a.saldo === undefined || b.saldo === undefined || a.currency !== b.currency) continue;
