...
export const membershipFormValidations = staticSuite((model: MembershipFormModel, field?: string) => {
  ...
  stringValidations('memberModelType', model.memberModelType, WORD_LENGTH); // person or org

  omitWhen(model.memberModelType !== 'person', () => {
    stringValidations('memberType', model.memberType, WORD_LENGTH);   // gender
  });
  omitWhen(model.memberModelType !== 'org', () => {
    stringValidations('memberType', model.memberType, WORD_LENGTH);   // orgType
  });
