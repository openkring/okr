...
import { WorkrelNewFormModel } from './workrel-new-form.model';

export const workrelNewFormValidations = staticSuite((model: WorkrelNewFormModel, field?: string) => {
  if (field) only(field);
  ...
  omitWhen(model.validFrom === '' || model.validTo === '', () => {
    test('validTo', '@workrelValidToAfterValidTo', () => {
