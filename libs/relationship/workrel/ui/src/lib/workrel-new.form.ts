...
import { WORKREL_NEW_FORM_SHAPE, WorkrelNewFormModel, workrelNewFormValidations } from '@bk2/relationship-workrel-util';
import { DEFAULT_CURRENCY, DEFAULT_DATE, DEFAULT_GENDER, DEFAULT_KEY, DEFAULT_LABEL, DEFAULT_NAME, DEFAULT_NOTES, DEFAULT_ORDER, DEFAULT_TAGS, DEFAULT_WORKREL_STATE, DEFAULT_WORKREL_TYPE } from '@bk2/shared-constants';
      ...
      <ion-card-content>
        <ion-grid>
          <ion-row>
            ...
            <ion-col size="12" size-md="6">
            <bk-text-input name="currency" [value]="currency()" [readOnly]="isReadOnly()" (changed)="onFieldChange('currency', $event)" />
            </ion-col>
            <ion-col size="12" size-md="6">
...
export class WorkrelNewFormComponent {
  // inputs
  public formData = model.required<WorkrelNewFormModel>();
  ...
  protected readonly shape = WORKREL_NEW_FORM_SHAPE;
  private readonly validationResult = computed(() => workrelNewFormValidations(this.formData()));
  ...
  protected objectType = computed(() => this.formData().objectType ?? DEFAULT_GENDER);
  ...
  protected price = computed(() => this.formData().price ?? 6000);
  ...
  protected onFormChange(value: WorkrelNewFormModel): void {
