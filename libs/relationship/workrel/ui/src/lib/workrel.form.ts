...
import { WorkrelFormModel, workrelFormModelShape, workrelFormValidations } from '@bk2/relationship-workrel-util';
import { DEFAULT_CURRENCY, DEFAULT_DATE, DEFAULT_GENDER, DEFAULT_KEY, DEFAULT_LABEL, DEFAULT_NAME, DEFAULT_NOTES, DEFAULT_ORDER, DEFAULT_ORG_TYPE, DEFAULT_PRICE, DEFAULT_PRIORITY, DEFAULT_TAGS, DEFAULT_WORKREL_STATE, DEFAULT_WORKREL_TYPE } from '@bk2/shared-constants';
      ...
      <ion-card-content>
        <ion-grid>
          <ion-row>
            ...
            <ion-col size="12" size-md="6">
            <bk-text-input name="currency" [value]="currency()" [maxLength]="3" [readOnly]="readOnly()" (changed)="onChange('currency', $event)" />
            </ion-col>
            <ion-col size="12" size-md="6">
...
export class WorkrelFormComponent {
  public vm = model.required<WorkrelFormModel>();
  ...
  protected objectType = computed(() => this.vm().objectType ?? DEFAULT_ORG_TYPE);
  ...
  private readonly validationResult = computed(() => workrelFormValidations(this.vm()));
