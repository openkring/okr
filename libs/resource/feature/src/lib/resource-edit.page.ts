...
import { ResourceModel, ResourceModelName, RoleName } from '@okr/shared-models';
...
import { DEFAULT_RESOURCE_TYPE, DEFAULT_TITLE } from '@okr/shared-constants';
...
@Component({
  selector: 'okr-resource-edit-page',
  standalone: true,
  imports: [
        ...
        <ion-grid>
          <ion-row>
            <ion-col size="12" size-md="6">
              <okr-cat-select [category]="types()!" [selectedItemName]="type()" (selectedItemNameChange)="onTypeChange($event)" [withAll]="false" [readOnly]="isReadOnly()" />
            </ion-col>
          </ion-row>
        </ion-grid>
      }
...
export class ResourceEditPage {
  ...
  // derived signals
  protected headerTitle = computed(() => this.getTitleLabel(this.isReadOnly(), this.resource()?.okey ?? '', this.type()));
