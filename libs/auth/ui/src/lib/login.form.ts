...
@Component({
  ...
  template: `
    <form scVestForm [formShape]="shape" [formValue]="vm()" [suite]="suite" (formValueChange)="onValueChange($event)">
      <ion-grid>
        <ion-row>
          <ion-col size="12">
            <bk-email name="loginEmail" [value]="vm().loginEmail" [showHelper]="true" [readOnly]="false" autocomplete="username email" />
