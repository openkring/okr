...
describe('/aoc guards after the 2026-08-05 rulings', () => {
  ...
  it('every /aoc child carries its own guard', () => {
    ...
    // 19 since the AOC diary screen was added (admin-only, not named by any ruling).
    expect(children.length, 'children were added or removed; re-check the ruling list').toBe(19);
  });
  ...
  it('every child the rulings do NOT name stays admin-only', () => {
    ...
    expect(others.length, 'expected sixteen admin-only /aoc children').toBe(16);
   ...
   * the union of these children: fourteen `['admin']` ∪ three `['contentAdmin', 'admin']`.
   */
